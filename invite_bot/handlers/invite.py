"""
FreshMinds Invite Competition Bot — Direct Channel Invite Link Generator
"""
...
async def handle_get_link(message: Message, bot: Bot):
    """Generate or retrieve direct native channel invite link (t.me/+...) with auto-approval."""
    ...
    # Only accept direct channel links starting with https://t.me/+ or https://t.me/joinchat
    if invite_link and not (
        invite_link.startswith("https://t.me/+")
        or invite_link.startswith("https://t.me/joinchat")
    ):
        invite_link = None

    # 2. Create native direct channel link with join-request for 100% accurate tracking
    if not invite_link:
        try:
            ...
            logger.info(
                f"Generated direct channel invite link for user {user.id}: {invite_link}"
            )
